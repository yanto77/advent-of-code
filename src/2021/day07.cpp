...
namespace
{
    std::pair<int32_t, int32_t> get_min_max(const std::vector<int32_t>& data)
    {
    ...
    int32_t get_fuel_part1(const std::vector<int32_t>& data, int32_t guess)
    {
        int32_t fuel = 0;
        for (auto pos: data)
        {
    ...
    int32_t get_fuel_part2(const std::vector<int32_t>& data, int32_t guess)
    {
        int32_t fuel = 0;
        for (auto pos: data)
        {
            int32_t n = abs(guess - pos);
            fuel += (n * (n + 1)) / 2;
        }
        return fuel;
    }

    std::pair<int32_t, int32_t> get_best_fuel(std::vector<int32_t> data)
    {
